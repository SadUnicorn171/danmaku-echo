...
const senderFor = {
  bilibili: sendBilibiliRichMessage,
  douyu: sendDouyuRichMessage,
  huya: sendHuyaRichMessage,
}
...
function runtime() {
  const context = {
    ...
    sendBilibiliNative: vi
      .fn<(payload: LiveRichMessagePayload) => Promise<boolean>>()
...
describe('platform rich-message senders', () => {
  it('sends Huya image Emoji through its bracket-name editor conversion', async () => {
    ...
    await expect(senderFor.huya(payload(), context)).resolves.toBe(true)
  ...
  it('keeps mixed Douyu image content on its bracket-name editor conversion', async () => {
    ...
    await expect(senderFor.douyu(payload('[开心]'), context)).resolves.toBe(true)
  ...
  it('sends an image-only Douyu Emoji through its native picker', async () => {
    ...
    await expect(senderFor.douyu(richPayload, context)).resolves.toBe(true)
  ...
  it('lets the Bilibili editor auto-recognize ordinary bracket Emoji', async () => {
    ...
    await expect(senderFor.bilibili(richPayload, context)).resolves.toBe(true)
  ...
  it.each(['huya', 'douyu'] as const)(
    ...
    async (platform) => {
      ...
      await expect(senderFor[platform](payload('[图片表情]'), context)).resolves.toBe(false)
