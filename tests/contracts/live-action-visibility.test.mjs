...
const hoverSelectionSource = readFileSync(
  resolve(root, 'src', 'platforms', 'live', 'hover-selection-controller.ts'),
  'utf8',
)
const capsuleControllerSource = readFileSync(
  resolve(root, 'src', 'platforms', 'live', 'capsule-controller.ts'),
  'utf8',
)
...
const douyuAdapterSource = readFileSync(
  resolve(root, 'src', 'platforms', 'douyu', 'adapter.ts'),
  'utf8',
)
...
const douyinPageApp = readFileSync(
  resolve(root, 'src', 'platforms', 'douyin', 'page', 'page-app.ts'),
  'utf8',
)
const douyinTrackController = readFileSync(
...
test('targets only Douyu native video-danmaku capsule controls', () => {
  ...
  assert.match(douyuAdapterSource, /capsuleTargets: findDouyuNativeDanmakuCapsuleTargets/)
  ...
  assert.match(sharedContentSource, /douyuBoundary\.capsuleDecorationSelectors/)
  ...
  assert.match(sharedContentSource, /douyuBoundary\?\.capsuleVisibility\.hide\(activeTargets\)/)
...
test('keeps the Douyu overlay capsule in the shared fixed portal', () => {
  ...
  assert.match(
    capsuleControllerSource,
    /bridge\.style\.setProperty\(['"]position['"], ['"]fixed['"], ['"]important['"]\)/,
  )
  assert.match(sharedContentSource, /const capsuleController = new CapsuleController/)
})

test('confines moving overlay hover and controls to the visible player viewport', () => {
  assert.match(sharedContentSource, /function overlayViewportRect\(candidate: Element\)/)
  assert.match(
    sharedContentSource,
  ...
  assert.match(
    capsuleControllerSource,
    /const actionViewport =[\s\S]{0,160}anchor\.overlayViewport \?\? browserViewport/,
  )
  assert.match(
    hoverSelectionSource,
    /!operations\.insideCurrentViewport\(point\)[\s\S]{0,80}operations\.clear\(\)/,
  )
  assert.match(hoverSelectionSource, /function pointerCoordinates\(event: MouseEvent\)/)
  assert.match(hoverSelectionSource, /!event\.isTrusted && x === 0 && y === 0/)
})
...
test('keeps every capsule divider on the same scalable width variable', () => {
  ...
  assert.match(douyinPageApp, /DOM_ACTION_DIVIDER_WIDTH\s*=\s*2/)
  assert.match(douyinTrackController, /snapToDevicePixel\(options\.actionItemWidths\[key\]\)/)
  assert.match(douyinTrackController, /snapToDevicePixel\(options\.actionDividerWidth\)/)
})
