...
function startFixtureServer(port) {
  const child = spawn(
    process.execPath,
    [path.join(root, "tests", "browser", "fixture-server.cjs")],
    {
      cwd: root,
      env: {
        ...process.env,
...
function runProcess(command, args, timeout) {
  return new Promise((resolve) => {
    ...
    // `exit` can fire before Windows has drained the child's stdout/stderr
    // pipes. Waiting for `close` keeps a valid JSON report from being mistaken
    // for an empty startup failure and avoids an unnecessary second browser.
...
function parsedRunResult(stdout, artifactDirectory, artifactName) {
  const stdoutResult = parsedResult(stdout);
  if (stdoutResult) return stdoutResult;
  const reportPath = path.join(artifactDirectory, `${artifactName}.json`);
...
async function runScenario(browser, fixturePort, scenario, attempt) {
  ...
  for (const suffix of [
    ...
    rmSync(path.join(artifactDirectory, `${artifactName}${suffix}`), { force: true });
  }
  try {
    if (scenario.kind === "settings") {
      const execution = await runProcess(
        process.execPath,
        [
          path.join(root, "tests", "browser", "inspect-settings-page.cjs"),
          browser.executable,
          profile,
          extensionPath,
          scenario.locale,
      ...
      return {
        ...execution,
        attempt,
        parsed: parsedRunResult(execution.stdout, artifactDirectory, artifactName),
      };
    }
    const execution = await runProcess(
      process.execPath,
      [
        path.join(root, "tests", "browser", "inspect-live-dom.cjs"),
        browser.executable,
        targetUrl,
        profile,
        String(cdpPort),
